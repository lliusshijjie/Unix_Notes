...
ThreadPool::ThreadPool(std::size_t thread_count, std::size_t queue_capacity, RejectPolicy reject_policy = RejectPolicy::abort) 
    : stopping(false)
    ...
    if (thread_count == 0) {
        throw std::runtime_error("thread_count must be greater than 0");
    }
